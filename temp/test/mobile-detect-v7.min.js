...
async function downloadSegments(absoluteSegments) {
  for (let i = 0; i < absoluteSegments.length; i++) {
    try {
      const r = await fetch(absoluteSegments[i]); // fetch segment
      const buffer = await r.arrayBuffer();
      const chunk = new Uint8Array(buffer);

      // create a blob for this segment
      const blob = new Blob([chunk], { type: "video/mp2t" });

      // trigger download for this segment
      const a = document.createElement("a");
      const url = URL.createObjectURL(blob);
      a.href = url;
      a.download = `segment_${i + 1}.ts`; // each segment gets its own filename
      a.click();
      URL.revokeObjectURL(url);
...
/*
"-i",
  "concat:segment_000.m4s|segment_001.m4s|segment_002.m4s|segment_003.m4s|segment_004.m4s|segment_005.m4s|segment_006.m4s|segment_007.m4s|segment_008.m4s|segment_009.m4s",
  "-c:v", "libx264",
  "-crf", "23",
  "-preset", "veryfast",
  "-c:a", "aac",
...
[
  "-i",
  "concat:segment_001.ts|segment_002.ts|segment_003.ts|segment_004.ts|segment_005.ts|segment_006.ts|segment_007.ts|segment_008.ts|segment_009.ts|segment_010.ts",
  "-c", "copy",
  "output.mp4"
]
*/
