(async () => {
  ...
  class LoadingScene {
    ...
    async createScene() {
      ...
      await PIXI.Assets.load([
        ...
        },
        {
          alias: "white-banner",
          src:
            "https://cdn.jsdelivr.net/gh/marcustansoon/PhysicsEngine.js-2D@master/temp/Games/BBQ-Sort/temp/white-image-rotated.jpg"
        }
      ]);
      ...
      // Calculate font size
      let fontSize = Math.round((this.scale / 0.3) * 24 * 10) / 10;
      ...
      mainMenu.scale.set(floor(this.scale, 2));
      ...
      if (window.localStorage.getItem("user-consent")) {
        this.acceptUserDataCollection();
      } else {
        this.createUserConsentPrompt();
      }
    ...
    createUserConsentPrompt() {
      // User data collection consent window
      let whiteBanner = PIXI.Assets.get("white-banner"),
        scaleWhiteBannerX = Math.min(
          0.7,
          (this.app.renderer.width / whiteBanner.width) * 0.9
        ),
        scaleWhiteBannerY =
          (this.app.renderer.height / whiteBanner.height) * 0.8,
        scaleWhiteBanner = Math.min(scaleWhiteBannerX, scaleWhiteBannerY);
      let consentWindow = new PIXI.Sprite(PIXI.Assets.get("white-banner"));
      consentWindow.scale.set(scaleWhiteBanner);
      consentWindow.anchor.set(0.5);
      consentWindow.position.set(
        this.app.screen.width / 2,
        this.app.screen.height / 2
      );
      this.objects.push(consentWindow);
      this.container.addChild(consentWindow);

      // Green banner
      let greenButton = new PIXI.Sprite(PIXI.Assets.get("green-banner"));
      greenButton.anchor.set(0.5);
      greenButton.scale.set(scaleWhiteBanner / 4);
      greenButton.position.set(
        this.app.screen.width / 2,
        this.app.screen.height / 2 + greenButton.height * 2
      );
      makeInteractive(greenButton);
      greenButton.on("pointerdown", () => {
        this.acceptUserDataCollection(true);
      });
      this.greenButton = greenButton;
      this.objects.push(greenButton);
      this.container.addChild(greenButton);

      let fontSizeUserDataConsent =
        Math.round((this.scale / 0.3) * 9.5 * 10) / 10;
      // Create text with custom style
      let styleUserDataConsent = new PIXI.TextStyle({
        fontFamily: "Trebuchet MS, sans-serif",
        fontSize: fontSizeUserDataConsent,
        fill: "#000000",
        dropShadow: false,
        fontWeight: "bold",
        wordWrap: true,
        wordWrapWidth: consentWindow.width * 0.8,
        letterSpacing: 1,
        lineJoin: "round"
      });
      let styleUserDataConsentAccept = new PIXI.TextStyle({
        fontFamily: "Trebuchet MS, sans-serif",
        fontSize: fontSizeUserDataConsent,
        fill: "#FFFFFF",
        dropShadow: false,
      ...
      let userDataConsentText = new PIXI.Text(
        "BBQ Sort collects and uses data about your device to enable core gameplay (e.g. saving your progress), tailor your game experience, provide you with customer service whenever necessary. To continue playing, we need your consent on the data collection",
        styleUserDataConsent
      );
      userDataConsentText.anchor.set(0.5);
      userDataConsentText.position.set(
        this.app.screen.width / 2,
        this.app.screen.height / 2 - greenButton.height
      );
      this.consentText = userDataConsentText;
      this.objects.push(userDataConsentText);
      this.container.addChild(userDataConsentText);

      // Accept data collection text
      let userDataConsentAcceptText = new PIXI.Text(
        "Accept",
        styleUserDataConsentAccept
      );
      userDataConsentAcceptText.anchor.set(0.5);
      userDataConsentAcceptText.position.set(
        this.app.screen.width / 2,
        this.app.screen.height / 2 + greenButton.height * 2
      );
      this.consentButtonText = userDataConsentAcceptText;
      this.objects.push(userDataConsentAcceptText);
      this.container.addChild(userDataConsentAcceptText);
    }

    acceptUserDataCollection(isUserClicked) {
      if (isUserClicked) {
        this.greenButton.interactive = false;
        this.greenButton.alpha = 0;
        this.consentText.alpha = 0;
        this.consentButtonText.alpha = 0;
      }
